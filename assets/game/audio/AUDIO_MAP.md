...
- [x] `music-match` — asset ✅ `music/battle-*.mp3`; runtime ✅ one randomly selected WorldXplore battle cue loops from match start until a winner is committed and resumes after the result music ends, with a persistent player-controlled volume slider in Settings.
- [x] `music-victory` — assets ✅ `music/music-victory-02-dawn-over-the-hexfield.mp3`; runtime ✅ Dawn plays once after winner commit; when it ends, the existing background playlist resumes and loops on the result screen. User retained Dawn only on 2026-09-06; Grave was removed for its beeping ending.
...
| `music-match` | `music/battle-*.mp3` (13 tracks) | Match scene starts; one randomly selected track loops until either player becomes the winner. After the result track ends, the same playlist supplies looping background music on the result screen; result-screen refreshes do not interrupt it. `?music=off` disables this lifecycle. | music | Player setting; 0.25 default | One track per match | Exact accepted WorldXplore runtime battle playlist; Settings persists and applies a 0–100% player volume, including mute at 0%; user requested reuse 2026-08-29. |
| `music-victory` | `music/music-victory-02-dawn-over-the-hexfield.mp3` | After `GRV1` winner commit, Dawn plays for the result presentation and does not loop. Natural completion resumes `music-match` as result-screen background music; stopping or disposing the result track cancels that handoff. `?music=off` disables this lifecycle. | music | 0.34 | One track per result | Sole local-victory result audio. User retained Dawn only on 2026-09-06 and rejected Grave for its beeping ending. The separate `match-victory` sting remains removed. |
