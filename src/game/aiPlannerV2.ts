...
interface SearchNode { state: GameState; actions: AiAction[]; score: number; }
interface CompletedPlan extends SearchNode { endedState: GameState; }
...
const budgetDiagnostics = (budget: SearchBudget): SearchPhaseDiagnostics => ({ nodes: budget.nodes, stopReason: budget.stopReason, legalActions: budget.legalActions, retainedActions: budget.retainedActions, legalByKind: { ...budget.legalByKind }, retainedByKind: { ...budget.retainedByKind } });
...
const stateSignature = (state: GameState, includeHand: boolean): string => {
  const units = state.units.map((unit) => [
    unit.id, unit.owner, unit.definitionId, unit.coord.q, unit.coord.r, unit.hp,
  ...
  return [state.currentPlayer, player.mana, includeHand ? player.hand.join(',') : '', units, sites,
    ...
    `${state.countdown?.player ?? 0}:${state.countdown?.checkpoints ?? 0}`, state.winner ?? 0].join(';');
};

const controlledDeploymentSites = (state: GameState, player: PlayerId) => state.sites.filter((site) => site.owner === player && (site.type === 'keep' || site.type === 'fort'));
const freeDeploymentSites = (state: GameState, player: PlayerId): number => controlledDeploymentSites(state, player).filter((site) => !state.units.some((unit) => sameCoord(unit.coord, site.coord))).length;
const blockedDeploymentSites = (state: GameState, player: PlayerId): number => controlledDeploymentSites(state, player).filter((site) => state.units.some((unit) => unit.owner === player && sameCoord(unit.coord, site.coord))).length;
const invadedDeploymentSites = (state: GameState, player: PlayerId): number => controlledDeploymentSites(state, player).filter((site) => state.units.some((unit) => unit.owner !== player && sameCoord(unit.coord, site.coord))).length;
...
const oneTurnLethalExposure = (state: GameState, player: PlayerId): number => {
  ...
  for (const target of state.units) {
    ...
    const threatened = state.units.some((attacker) => {
      ...
      return definition.attack >= target.hp && hexDistance(attacker.coord, target.coord) <= definition.move + definition.range + hillRange;
    });
...
const evaluateV2 = (state: GameState, perspective: PlayerId): StrategicEvaluation => {
  ...
  const deploymentDelta = (ownFree - enemyFree) * 2.5 - (ownBlocked - enemyBlocked) * 2.2 - (ownInvaded - enemyInvaded) * 5.5;
  ...
  const bankedPlayableMana = state.currentPlayer === perspective && ownPlayableUnits > 0 ? -Math.min(5, state.players[perspective].mana * 0.55) : 0;
  components.economy = clamp(components.economy + deploymentDelta + deploymentEmergency + handHoardingCorrection + bankedPlayableMana, -20, 20);
  const exposureDelta = oneTurnLethalExposure(state, perspective) - oneTurnLethalExposure(state, opponent);
  components.position = clamp(components.position - clamp(exposureDelta / 420, -10, 10) - ownInvaded * 3 + enemyInvaded * 3, -24, 24);
  return { outlook: clamp(Object.values(components).reduce((sum, value) => sum + value, 0), -100, 100), components };
};

const actionSequenceUtility = (initial: GameState, state: GameState, actions: AiAction[], actor: PlayerId): number => {
...
const actionPriority = (state: GameState, actor: PlayerId, action: AiAction): number => {
  ...
  if (action.kind === 'attack') {
    ...
    return (target.definitionId === 'commander' ? 100_000 : 0) + (lethal ? 12_000 : 0) + strategicUnitValue(target);
  }
  if (action.kind === 'move') {
    ...
    const approach = enemyCommander ? (hexDistance(unit.coord, enemyCommander.coord) - hexDistance(action.destination, enemyCommander.coord)) * 320 : 0;
    const definition = unitDefinition(unit);
    const setsUpAttack = state.units.some((target) => target.owner === opponent && hexDistance(action.destination, target.coord) <= definition.range) ? 2_800 : 0;
    return freesBlockedSite + capture + setsUpAttack + approach;
  }
  if (action.kind === 'summon') {
    const card = CARD_DEFINITIONS[action.cardId];
    const enemyCommander = state.units.find((candidate) => candidate.owner === opponent && candidate.definitionId === 'commander');
    return 3_000 + card.cost * 420 + (enemyCommander ? Math.max(0, 12 - hexDistance(action.destination, enemyCommander.coord)) * 80 : 0);
  }
...
const iterativeTurnSearch = (initialState: GameState, actor: PlayerId, perspective: PlayerId, includeCards: boolean, maximize: boolean, beamWidth: number, maxDepth: number, planCount: number, budget: SearchBudget): SearchResult => {
  ...
  for (let depth = 1; depth <= maxDepth; depth += 1) {
    ...
    for (const node of beam) {
      if (budgetExhausted(budget)) { depthCompleted = false; break; }
      ...
      for (const action of actions) {
        ...
        next.push({ state: child, actions: childActions, score });
      }
    ...
    const finished = beam.map((node) => {
      ...
      return { ...node, endedState, score: evaluateV2(endedState, perspective).outlook + (actor === perspective ? utility : -utility) };
    });
    bestCompleted = [...bestCompleted, ...finished].sort((a, b) => maximize ? b.score - a.score : a.score - b.score).slice(0, Math.max(planCount * 3, beamWidth));
  }
  ...
  return { plans: bestCompleted.slice(0, planCount), diagnostics: budgetDiagnostics(budget), completedDepth, rootHadActions };
};

const material = (state: GameState, player: PlayerId): number => state.units.filter((unit) => unit.owner === player).reduce((sum, unit) => sum + strategicUnitValue(unit), 0);
const makeUnassessedTactical = (state: GameState, perspective: PlayerId): TacticalAssessment => {
  ...
  return { ...base, tier: base.tier === 'forced-win' ? 'forced-win' : 'unsafe', score: base.tier === 'forced-win' ? base.score : -85, worstResponse: [], worstResponseStrategicOutlook: base.tier === 'forced-win' ? base.worstResponseStrategicOutlook : -100 };
};
const strengthenTacticalTier = (candidateState: GameState, responseState: GameState, perspective: PlayerId, assessment: TacticalAssessment): TacticalAssessment => {
  ...
  const lostUnit = candidateState.units.some((unit) => unit.owner === perspective && unit.definitionId !== 'commander' && !responseState.units.some((after) => after.id === unit.id));
  if (!lostUnit || lostMaterial < 240) return assessment;
  return { ...assessment, tier: 'unsafe', score: Math.min(-18, assessment.score - Math.min(55, lostMaterial / 20)) };
};
const tacticalRank = (assessment: TacticalAssessment): number => assessment.tier === 'forced-win' ? 3 : assessment.tier === 'safe' ? 2 : assessment.tier === 'unsafe' ? 1 : 0;
const compareResponses = (left: { plan: CompletedPlan; tactical: TacticalAssessment }, right: { plan: CompletedPlan; tactical: TacticalAssessment }): number => {
  const rank = tacticalRank(left.tactical) - tacticalRank(right.tactical);
  return rank !== 0 ? rank : left.tactical.worstResponseStrategicOutlook - right.tactical.worstResponseStrategicOutlook;
};

const assessCandidates = (initialState: GameState, candidates: CompletedPlan[], perspective: PlayerId, options: Required<AiSearchOptions>): { candidates: AssessedCandidate[]; diagnostics: SearchPhaseDiagnostics; maxResponseDepth: number } => {
  ...
  for (const candidate of candidates) {
    if (candidate.endedState.winner || candidate.endedState.currentPlayer !== responder) {
      assessed.push({ candidate, strategic: evaluateV2(candidate.endedState, perspective), tactical: assessTacticalOutcome(initialState, candidate.endedState, perspective, []), responseDepth: 0, assessed: true });
      continue;
    }
    const directBudget = createBudget(Math.max(20, perCandidateNodes), Math.max(8, perCandidateMs));
    const directActions = selectV2Actions(candidate.endedState, responder, false, directBudget).filter((action) => action.kind === 'attack');
    ...
    for (const action of directActions) {
      ...
      const tactical = strengthenTacticalTier(candidate.endedState, ended, perspective, assessTacticalOutcome(initialState, ended, perspective, [action]));
      const wrapper = { plan: { state: ended, endedState: ended, actions: [action], score: tactical.worstResponseStrategicOutlook }, tactical };
      if (!worstDirect || compareResponses(wrapper, { plan: { state: worstDirect.state, endedState: worstDirect.state, actions: worstDirect.actions, score: worstDirect.tactical.worstResponseStrategicOutlook }, tactical: worstDirect.tactical }) < 0) {
        worstDirect = { state: ended, actions: [action], tactical };
      }
    }
    const responseBudget = createBudget(perCandidateNodes, perCandidateMs);
    const response = iterativeTurnSearch(candidate.endedState, responder, perspective, false, false, options.responseBeamWidth, options.responseDepth, 4, responseBudget);
    ...
    const responseOptions = response.plans.map((plan) => ({ plan, tactical: strengthenTacticalTier(candidate.endedState, plan.endedState, perspective, assessTacticalOutcome(initialState, plan.endedState, perspective, plan.actions)) }));
    if (worstDirect) responseOptions.push({ plan: { state: worstDirect.state, endedState: worstDirect.state, actions: worstDirect.actions, score: worstDirect.tactical.worstResponseStrategicOutlook }, tactical: worstDirect.tactical });
    ...
    assessed.push({
      ...
      tactical: worst ? (validAssessment || worst.tactical.tier !== 'safe' ? worst.tactical : makeUnassessedTactical(candidate.endedState, perspective)) : (validAssessment ? assessTacticalOutcome(initialState, candidate.endedState, perspective, []) : makeUnassessedTactical(candidate.endedState, perspective)),
...
const compareCandidates = (left: AssessedCandidate, right: AssessedCandidate): number => {
  ...
  if (right.candidate.actions.length !== left.candidate.actions.length) return right.candidate.actions.length - left.candidate.actions.length;
...
export const planSmartAiTurnV2 = (state: GameState, overrides: AiSearchOptions = {}): AiPlan => {
  ...
  if (state.winner || perspective !== 2) {
    ...
    return { actions: [], strategic, tactical: assessTacticalOutcome(state, state, 2), diagnostics: { strategy: emptyDiagnostics(), tactical: emptyDiagnostics() } };
  }
  const strategyBudget = createBudget(options.strategyMaxNodes, options.strategyMaxPlanningMs);
  const strategy = iterativeTurnSearch(state, perspective, perspective, true, true, options.beamWidth, options.maxDepth, Math.max(2, options.candidatePlans), strategyBudget);
